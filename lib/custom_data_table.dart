...
import 'custom_adjust_headerIndex.dart';
import 'custom_adjust_row_index.dart';
...
class CustomDataRow {
  ...
  CustomDataRow.byIndex({
    ...
    this.color,
    required this.cells,
  }) : key = ValueKey<int?>(index); //assert(cells != null),
...
class CustomDataTable extends StatefulWidget {
  ...
  final Function(int column, int showColumnNumber) onSortColumn;
  int showColumnNumber;
  final Function(int showColumnNumber) setshowColumnNumber;
  final double Function(int index) getColumnsWidth;
  final String? addColumnTooltip;
  final double height;
  final bool selectAllOnPage;
  final isSelectedAll;
  final Function(bool) selectAllhandler;
  ...
  CustomDataTable({
    ...
    // this.onSelectAll,
    required this.selectAllOnPage,
    required this.isSelectedAll,
    required this.selectAllhandler,
    this.decoration,
    this.dataRowColor,
    this.dataRowHeight,
    this.dataTextStyle,
...
class _CustomDataTableState extends State<CustomDataTable> {
  ...
  void _handleSelectAll(bool? checked, ValueNotifier<bool> allChecked) {
    ...
    if (widget.selectAllOnPage) {
      for (final CustomDataRow row in widget.rows) {
        (row.onSelectChanged ?? (_) {})(allChecked.value);
      }
    } else {
      widget.selectAllhandler(allChecked.value);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final ValueNotifier<bool> allChecked = ValueNotifier<bool>(widget
            .selectAllOnPage
      ...
      for (final CustomDataRow row in widget.rows) {
        ...
        tableRows[rowIndex].children![0] = _buildCheckbox(
          ...
          onCheckboxChanged: (value) {
            ...
            allChecked.value = widget.selectAllOnPage
                ? widget.rows.every((row) => row.selected.value)
                : widget.isSelectedAll();
            // print(widget.isSelectedAll());
          },
    ...
    //TODO: Use this to delete column by drag-drop
...
// TODO: Use This to build AddColumnCheckbox
    ...
    List<Widget> headerMenu = List.generate(
        ...
        (index) => buildAddColumnBox(
            ...
            shownHeader[widget.customColumnsIndex[index]],
            widget.columns[widget.customColumnsIndex[index]].label));
    ...
    Widget addColumnButton = PopupMenuButton<int>(
      ...
      onCanceled: () {
        ...
        for (int index in toRemove.reversed) {
          widget.customColumnsIndex.insert(widget.showColumnNumber, index);
        }
