...
class CustomPaginatedDataTable extends StatefulWidget {
  ...
  ///
  final List<int> customColumnsIndex;
  final List<int>? sortedIndexList;
  final List<int> allSortedIndexList = <int>[];
  final int Function(int a, int b, int column) onSort;
...
class CustomPaginatedDataTableState extends State<CustomPaginatedDataTable> {
  ...
  // void selectAllHandler(bool select) {
  //   _getRows(0, widget.source.rowCount)
  //       .forEach((row) => (row.onSelectChanged ?? (_) {})(select));
  // }

  bool isSelectedAll() {
    return widget.source.rowCount == widget.source.selectedRowCount;
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.allSortedIndexList.isEmpty) {
      widget.allSortedIndexList
          .addAll(List.generate(widget.source.rowCount, (index) => index));
    }
            ...
            children: <Widget>[
              ...
              ConstrainedBox(
                ...
                child: CustomDataTable(
                  ...
                  selectAllOnPage: widget.selectAllOnPage,
                  selectAllhandler: widget.selectAllHandler ??
                      (bool select) {
                        _getRows(0, widget.source.rowCount).forEach(
                            (row) => (row.onSelectChanged ?? (_) {})(select));
                      },
